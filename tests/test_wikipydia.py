...
import src.wikipydia
import mocks


class TestWikipydia(unittest.TestCase):

    @mock.patch('requests.Session.get', side_effect=mocks.mock_wikipydia_query_random)
    def test_query_random(self, _mock):
        result = src.wikipydia.Wikipydia.query_random()
        self.assertEqual(result['mock']['random'][0]['title'], '1974 in Northern Ireland')

    @mock.patch('requests.Session.get', side_effect=mocks.mock_wikipydia_query_search)
    def test_query_search(self, _mock):
        result = src.wikipydia.Wikipydia.query_search()
    ...
    @mock.patch('requests.Session.get', side_effect=mocks.mock_wikipydia_query_property)
    def test_query_property(self, _mock):
        result = src.wikipydia.Wikipydia.query_property()
        print(result)
